...
const PropertyIcons = {
  ...
  rooms: {
    ...
    value: (rooms, isRU) => (isRU ? rooms.ru : rooms.en),
  },
  bathroom: {
    icon: (
      <svg
...
function PropCard({ el, isGrid, isRU }) {
  ...
  const title = isRU ? titleRussian : titleEnglish;
  const description = isRU ? descriptionRussian : descriptionEnglish;
  const state = isRU ? stateRussian : stateEnglish;
  const rooms = { en: roomsEnglish, ru: roomsRussian };

  const formatPrice = (price) =>
    `€ ${Number(price).toLocaleString("en-US").replace(/,/g, ",")}`;

  const truncateDescription = (text) =>
    text.length > 200 ? `${text.slice(0, 287)}...` : text;
  ...
  return (
    ...
    >
      ...
      <div className={`${isGrid ? "" : "flex w-2/3"}`}>
        <div
          className={`text-black dark:text-white ${isGrid ? "p-6" : "p-6 sm:w-1/2"}`}
        >
          ...
          <ul className="py-6 border-y border-slate-100 dark:border-gray-800 flex items-center justify-between list-none">
            {Object.entries(PropertyIcons).map(([key, { icon, value }]) => (
              <li key={key} className="flex items-center me-4">
                {icon}
